...
- **Application secrets and integrations** — `DATABASE_URL`, session signing material, Replit connector identity tokens, Google Sheets access tokens, and Resend API credentials. Compromise would expose data and let attackers abuse external integrations.
...
- **API to third-party services** — the server calls Google Sheets, Expo Push, Apple/Google identity services, and Resend using server-held credentials. These calls must not be attacker-steerable, and identity assertions must be bound safely to local accounts.
...
- **Highest-risk areas:** session setup and auth/account-linking routes in `server/routes.ts`; startup/bootstrap logic in `server/index.ts` and `server/seed.ts`; public voting flow in `server/routes.ts` and `server/templates/vote.html`; authenticated outbound-integrations routes such as `/api/prompts/sync` and hangout invite email sending
- **Public surface:** `/vote/:shareCode`, `GET/POST /api/vote/:shareCode`, unauthenticated calendar download route
- **Authenticated surface:** `/api/auth/me`, `/api/auth/profile`, `/api/contacts*`, `/api/hangouts*`, `/api/prompts*`, notification token updates, authenticated email-invite sending
