...
GitHub Actions schedules can also arrive hours late. A short scheduled job that checks the wall clock can be marked successful while skipping its health request, even though it missed the notification window entirely. Start each scheduled job several hours early and let it wait for and own exactly one 8–10 AM or 4–6 PM Toronto window. Keep manual dispatch as an immediate one-shot health check.
...
**How to apply:** Verify the health step itself ran; a green workflow conclusion is insufficient. Preserve enough early-start buffer for GitHub queue delays while keeping each job below the six-hour runner timeout. Split morning and evening into distinct schedule expressions, and exit successfully after the assigned window instead of sleeping toward the next one.